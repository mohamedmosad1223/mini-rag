from fastapi import APIRouter,FastAPI,status,Request,Depends,UploadFile
...
async def index_project(
    ...
    # setup batching 
    total_chunk_count= await chunk_model.get_total_chink_count(
    ...
    while has_records:
        ...
        if not is_inserted:
            return JSONResponse(
                ...
                content={
                    "signal":ResponseSignal.INSERT_INTO_VECTOREDB_FAILED.value
                }
            )
    ...
    return JSONResponse(
        content={
            "signal":ResponseSignal.INSERT_INTO_VECTOREDB_SUCCESS.value
            ,"inserted_count":inserted_item_count
        }
    )
...
async def get_projectindex_info(request:Request,project_id:str):
    project_model =await ProjectModel.create_instance(
        db_client=request.app.db_client,
    )
