...
# when it init create indexes
    ...
    #     if DataBaseEnum.COLLECTION_CHUNK_NAME.value not in all_collectiona:
    ...
    async def get_chunks(self, chunk_id):
        async with self.db_client() as session:
            async with session.begin():
    ...
    async def getproject_chunk(self, project_id: ObjectId,page_no:int=1,page_size:int=50):
        async with self.db_client() as session:
            async with session.begin():
    ...
    async def get_total_chink_count(self,project_id :ObjectId):
        total_count=0
        async with self.db_client() as session:
